 ...
 */
export function addHighlightLayers(map: maplibregl.Map, style: Required<GeoJsonStyle>) {
 ...
 */
export function updateHoverHighlight(map: maplibregl.Map, featureId: string | number | null) {
  const filter = featureId !== null ? ['==', ['id'], featureId] : ['==', ['get', '__hover'], true]; // Matches nothing

  for (const layerId of [HOVER_FILL_LAYER, HOVER_LINE_LAYER, HOVER_POINT_LAYER]) {
 ...
 */
export function updateSelectionHighlight(map: maplibregl.Map, featureIds: (string | number)[]) {
  const filter = featureIds.length > 0 ? ['in', ['id'], ['literal', featureIds]] : ['==', ['get', '__selected'], true]; // Matches nothing

  for (const layerId of [SELECTED_FILL_LAYER, SELECTED_LINE_LAYER, SELECTED_POINT_LAYER]) {
    if (map.getLayer(layerId)) {
      map.setFilter(layerId, filter as maplibregl.FilterSpecification);
    }
  }
