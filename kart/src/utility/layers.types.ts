...
const isWhitelist = (layers: VisibleLayersConfig): layers is Whitelist => Object.hasOwn(layers, 'whitelist');

export interface MapDefaultLayersProps {
  layers?: VisibleLayersConfig;
}

export const isLayerVisible = (layer: MapLayer, layers?: VisibleLayersConfig) => {
  if (!layers) {
    return true;
  }
