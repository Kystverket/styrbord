...
export function useCompassMarker({
  ...
  const getOrCreateMarkerElement = useCallback(() => {
    ...
    const onPointerDown = (e: PointerEvent) => {
      ...
      onDirectionChangeRef.current?.({ ...currentValueRef.current, direction: rounded });
    };
...
function positionHandle(compass: HTMLElement, handle: HTMLElement, dirDeg: number): void {
