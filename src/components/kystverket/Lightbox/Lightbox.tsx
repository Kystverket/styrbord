  ...
  return (
    <dialog ref={dialogRef} className={classes.frame} onClose={handleClose}>
        ...
        <Box horizontal gap={32}>
          <Box horizontal gap={8}>
            ...
            {buttonConfig.showDownload && (
              <Button title="Last ned" variant="filled" color="neutral" icon onClick={handleDownload}>
      ...
      <Box className={classes.content}>
        ...
        {files.map((e, idx) => (
          <PreviewFile file={e} chosen={idx === selectedFileIndex} />
        ))}
      </Box>
...
function PreviewFile({ file, chosen }: { file: FileInfo; chosen: boolean }) {
  switch (file.contentType) {
    case 'image': {
      return <img className={`${classes.previewFile} ${chosen ? '' : classes.none}`} src={file.src}></img>;
    }
    case 'json': {
      return <JsonDisplay chosen={chosen} data={file.data} />;
    }
    case 'pdf': {
      return <embed className={`${classes.previewFile} ${chosen ? '' : classes.none}`} src={file.src}></embed>;
    }
  }
}

function JsonDisplay({ data, chosen }: { data: Record<string, unknown>; chosen: boolean }) {
  return (
    <pre
      className={`${classes.previewFile} ${chosen ? '' : classes.none}`}
      style={{
