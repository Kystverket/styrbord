...
import {
  EXPERIMENTAL_MultiSuggestion as MultiSuggestion,
  Button,
  Divider,
  Field,
  Label,
...
export default {
  title: 'Designsystemet/MultiSuggestion',
...
const DATA_PLACES = ['Sogndal', 'Oslo', 'Stavanger', 'Brønnøysund', 'Trondheim', 'Bergen'];
...
export const Controlled: StoryFn<typeof MultiSuggestion> = (args) => {
  ...
  return (
    <>
      ...
      <Paragraph style={{ margin: 'var(--ds-size-2) 0' }}>Valgte reisemål: {value.join(', ')}</Paragraph>
  ...
  return (
    <Field>
      ...
      <MultiSuggestion {...args} filter={false}>
        <MultiSuggestion.Chips />
        <MultiSuggestion.Input value={value} onChange={(event) => setValue(event.target.value)} />
        <MultiSuggestion.Clear />
        <MultiSuggestion.List>
          <MultiSuggestion.Empty>Tomt</MultiSuggestion.Empty>
          {DATA_PLACES.filter((_, index) => !value || index === Number(value) - 1).map((text) => (
    ...
    <Field lang="en">
      ...
      <MultiSuggestion {...args} filter={false}>
        ...
        <MultiSuggestion.List singular="%d country" plural="%d countries">
          ...
          {options?.map((option) => <MultiSuggestion.Option key={option}>{option}</MultiSuggestion.Option>)}
        </MultiSuggestion.List>
      </MultiSuggestion>
    </Field>
  );
